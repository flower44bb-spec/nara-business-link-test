...
const collaborations = [
  { title: "マルシェ共同出店メンバー募集", from: "農業", area: "天理市", detail: "食品・雑貨・体験系の事業者を募集。" },
  { title: "奈良県産品の共同商品開発", from: "食品製造", area: "橿原市", detail: "地元素材を使った新商品を作りたい。" },
  { title: "県青連フェス出展者募集", from: "サービス業", area: "奈良市", detail: "年度末フェスで一緒に出展できる事業者を募集。" },
];

const successes = [
  { tag: "IT・Web × 飲食業", title: "ホームページ制作の相談から受注が成立", result: "HP制作受注", text: "飲食店の集客課題に対し、青年部内のWeb制作事業者とマッチング。" },
  { tag: "農業 × 飲食業", title: "地元野菜の仕入れ連携が成立", result: "継続取引見込み", text: "販路拡大を希望する農家と、地元食材を使いたい飲食店が連携。" },
  { tag: "建設業 × 小売業", title: "店舗改装の相談から見積依頼へ", result: "商談成立", text: "店舗改装を検討していた小売業者に青年部内の建設業者を紹介。" },
];
  ...
  <a href="/successes">🏆<h3>成功事例</h3><p>実績を共有</p></a>
      ...
      <section className="two">
        ...
        <div className="panel" id="login">
          ...
          <p className="note">※現在はデモ画面です。実運用時はSupabaseで認証管理します。</p>
        </div>
      </section>
      ...
      <section className="two" id="collab">
        <div className="panel">
          ...
          <button type="button">募集を投稿する</button>
        </div>

        <div className="panel">
          <h2>コラボ募集一覧</h2>
          {collaborations.map((c) => (
            <article className="post" key={c.title}>
              <span>コラボ募集</span>
              <h3>{c.title}</h3>
              <p>{c.from} ／ {c.area}</p>
              <p>{c.detail}</p>
